defmodule Commanded.EventStore.Adapters.Extreme.Subscription do
  ...
  def start_link(event_store, stream, subscription_name, subscriber, serializer, opts) do
    state = %State{
      ...
      message_timeout_milliseconds: Keyword.get(opts, :message_timeout_milliseconds, 10_000),
      record_statistics: Keyword.get(opts, :record_statistics, false),
      live_buffer_size: Keyword.get(opts, :live_buffer_size, 500),
      read_batch_size: Keyword.get(opts, :read_batch_size, 20),
      buffer_size: Keyword.get(opts, :buffer_size, 500),
      max_retry_count: Keyword.get(opts, :max_retry_count, 10),
      prefer_round_robin: Keyword.get(opts, :prefer_round_robin, false),
      checkpoint_after_time: Keyword.get(opts, :checkpoint_after_time, 1_000),
      checkpoint_max_count: Keyword.get(opts, :checkpoint_max_count, 500),
      checkpoint_min_count: Keyword.get(opts, :checkpoint_min_count, 1),
  ...
  defp create_persistent_subscription(%State{} = state) do
    %State{
      ...
      checkpoint_after_time: checkpoint_after_time,
